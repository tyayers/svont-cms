...
func InitializeProvider() data.PostIndex {
	...
	postBytes, err = dataProvider.DownloadFile("index_users.json")
...
func FinalizeProvider(persistMode data.PersistMode, index data.PostIndex) {
	...
	// Persist users
	if persistMode == data.PersistAll || persistMode == data.PersistOnlyUsers {
		jsonData, err := json.Marshal(index.IndexUsers)
		if err != nil {
			fmt.Printf("could not marshal json: %s\n", err)
			return
		}
